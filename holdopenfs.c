/*
  ...
  Holds all accessed files open, does not close any FDs.
  When opening file with the same name, FD is reused.
  Access mode is used from the first open.

  Based on fusexmp_fh.c

  gcc -Wall `pkg-config fuse --cflags --libs` -lulockmgr holdopenfs.c -o holdopenfs
*/
